...
console.log("okofk");
	...
	const getCategoryPosts = async (page = 1) => {
		...
		const categoryPosts = await api.get('/post/list/'+page, { params: {'post_categories.category_name': props.match.params.categoryName},headers: {'auth-token':localStorage.getItem('auth-token')}})
