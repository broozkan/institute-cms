...
class UpdateSlider extends React.Component {
    ...
    async componentDidMount() {
        ...
        // get user list
        await getSliderList(this.props.match.params.sliderId, (sliderData) => {
    ...
    handleSubmit = async (value) => {
        ...
        await axios.put(process.env.REACT_APP_API_ENDPOINT+'/slider/update/' + this.props.match.params.sliderId, { formData })
            .then(res => {
                if (res.data.response == true) {
                    Swal.fire({
                        title: 'Başarılı!',
                        text: 'Slider düzenlendi',
                        icon: 'success',
    ...
    render() {
        ...
        let sliderFormContent = ""

        if (this.state) {
            sliderFormContent = <SliderForm states={this.state} formSubmitFunction={this.handleSubmit} />
        }
